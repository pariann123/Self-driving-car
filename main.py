...
Original file is located at
    https://colab.research.google.com/drive/1iqcmom72Ap3IliwmlgRpQB7r9Kc4WoDC
"""
...
class Road:
  ...
  def __init__(self, check=False):    
    ...
    self.yolo = YoloV3(classes=8, yolo_max_boxes=5, yolo_iou_threshold=0.03, yolo_score_threshold=0.03) #0.03
  ...
  def Predict(self, image):
    ...
    if self.check:
      ...
      axs[1].imshow(road); axs[0].imshow(lanes>0.5)
...
#from autopilot.models.rollin_in_the_deep.yolo import YoloV3


class Model:
    saved_yolo = 'yolo.tf'
    saved_nvidia = 'nvidia.h5'
    saved_unet = 'unet.h5'
    saved_left = 'left.h5'
    saved_right = 'right.h5'
    

    def __init__(self):
        '''
        # models path
        yolo_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), self.saved_yolo)
        nvidia_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), self.saved_nvidia)
        unet_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), self.saved_unet)
        left_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), self.saved_left)
        right_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), self.saved_right)
        
        # object detection YOLO
        self.yolo = YoloV3(classes=8, yolo_max_boxes=5, yolo_iou_threshold=0.03, yolo_score_threshold=0.03)
        self.yolo.load_weights(yolo_path).expect_partial()
        
        # road segmentation UNET
        self.H_unet=128
        self.unet=keras.models.load_model(unet_path, custom_objects={'dice_coefficient':0})
        
        # angle prediction NVIDIA
        self.H_nvidia, self.W_nvidia=88, 128
        self.nvidia=keras.models.load_model(nvidia_path)
        self.left=keras.models.load_model(left_path)
        self.right=keras.models.load_model(right_path)
        '''
        # object detection YOLO
        self.yolo = YoloV3(classes=8, yolo_max_boxes=5, yolo_iou_threshold=0.03, yolo_score_threshold=0.03) #0.03
        self.yolo.load_weights('./weights/yolo.tf').expect_partial()
    ...
    def Mask(self, mask, angle, THR=0.7):
        ...
        first_lane=False
        prev_x1, prev_x2= 0, N
        for y in np.flip(np.arange(0, M-10, step)):
          x1=0; x2=N
          # find left lane
          x=x_start
          while (x>1)*(x1==0):
...
angle_model, speed_model=model.predict(img)
t2=time.perf_counter()
print(n)
print('time=', t2-t1)
print('prediction: speed=', speed, 'angle=', angle)
print('target: speed=', SPEED[n-1], 'angle=', ANGLE[n-1])
print( 'speed model=', speed_model, 'angle model=',  angle_model)
...
speed_df.to_csv("/content/results_05_05.csv", header=True, index=False)
...
results_df=pd.read_csv('/content/results_05_05.csv', delimiter=',')
...
print(speed_df.dtypes)
# save the dataframe as a csv file
speed_df.to_csv("/content/results_06_05.csv", header=True, index=False)
